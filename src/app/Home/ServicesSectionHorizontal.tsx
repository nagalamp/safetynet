...
const buttonVariants = { initial: { scale: 1, rotate: -45, backgroundColor: '#47d4aa' }, hover: { scale: 1.1, rotate: 0, backgroundColor: '#f05623', transition: { duration: 0.3, ease: 'easeInOut' } } };
...
export default function ServicesSectionHorizontal() {
  ...
  const scrollBy = (offset: number) => {
    if (carouselRef.current) carouselRef.current.scrollBy({ left: offset, behavior: 'smooth' });
  };

  return (
    <motion.section className="w-full py-12 flex flex-col items-center" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={staggerContainer}>
        ...
        <div className="relative w-full overflow-hidden">
          ...
          >
            {[...serviceData, ...serviceData].map((service, index) => (
              <motion.div key={index} className="relative flex-shrink-0 w-[280px]" variants={cardVariants}>
                <div className="relative w-full h-[438px] rounded overflow-hidden cursor-pointer" onClick={() => !isDragging && handleServiceClick(service)}>
                  
                  {/* Image */}
                  <motion.div className="absolute inset-0 w-full h-full" variants={imageVariants} initial="initial" whileHover="hover">
                    <Image src={service.image} alt={service.title} fill className="object-cover" priority={index < serviceData.length} />
                  ...
                  <motion.div className="absolute inset-0 w-full h-full bg-[#2c0087] rounded p-6 flex flex-col justify-start" variants={cardContentVariants} initial="initial" whileHover="hover">
                    ...
                    <div className="space-y-3 flex-1 mt-2">
                      {service.features?.map((feature, i) => (
                        <motion.div key={i} className="flex flex-row gap-3 justify-start items-start w-full" variants={featureItemVariants}>
          ...
          <div className="absolute top-full mt-6 left-1/2 transform -translate-x-1/2 flex gap-4 z-10 flex-wrap justify-center">
            <button className="px-4 py-2 bg-[#47d4aa] rounded text-white font-semibold" onClick={() => scrollBy(-300)}>Back</button>
            <button className="px-4 py-2 bg-[#47d4aa] rounded text-white font-semibold" onClick={() => scrollBy(300)}>Forward</button>
          </div>
        </div>
      </div>
    </motion.section>
